...
export default function ProductosParticularesPage() {
  ...
  // Obtener productos particulares
  useEffect(() => {
    setLoading(true);

    let url = `${
      process.env.NEXT_PUBLIC_STRAPI_URL
    }/api/products?sort=${encodeURIComponent(
      sort
    )}&pagination[page]=${page}&pagination[pageSize]=${PAGE_SIZE}&filters[provider][tipo][$eq]=Particular&populate[images]=true&populate[provider][populate]=image`;
    ...
    url += `&filters[provider][tipo][$eq]=Particular`;
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        setHasMore(data.data && data.data.length === PAGE_SIZE);
        setLoading(false);
      })
      .catch((error) => {
